...
from .models import CustomUser, InternshipPlacement, WeeklyLog, Evaluation, Student, Supervisor, Feedback, SiteSetting, Notification, StatusHistory, StudentGrade, EvaluationCriteria
...
def list_supervisors(request):
    ...
    for supervisor in supervisors:
        ...
        data['assigned_students_count'] = supervisor.assigned_students.count() #type: ignore
...
def assign_supervisor(request):
    ...
    student.assigned_supervisor = supervisor #type: ignore
    student.save()
...
def review_weekly_log(request, log_id):
    ...
    student_profile = getattr(weekly_log.user, 'student', None)
    if student_profile is None or student_profile.assigned_supervisor_id != supervisor.id: #type: ignore
        return Response(
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_log_history(request, log_id):
    try:
        log = WeeklyLog.objects.get(id=log_id)
    except WeeklyLog.DoesNotExist:
        return Response({"error": "Log not found"}, status=status.HTTP_404_NOT_FOUND)

    permission_error = require_role(request.user, ['student', 'supervisor', 'admin'])
    if permission_error:
        return permission_error

    
    history = StatusHistory.objects.filter(log=log).select_related('changed_by').order_by('-changed_at')
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_student_grades(request, criteria_id):
    try:
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def create_student_grade(request, criteria_id):
    try:
        criteria = EvaluationCriteria.objects.get(id=criteria_id)
    ...
    data['criteria'] = criteria.id #type: ignore
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_student_grade(request, grade_id):
    try:
